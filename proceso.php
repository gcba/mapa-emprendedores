...
if (!empty($_REQUEST['captcha_txt'])) {
    if (empty($_SESSION['captcha']) || trim(strtolower($_REQUEST['captcha_txt'])) != $_SESSION['captcha']) {
        ...
        echo "B";
        $columnas_db = "the_geom,nombre,descripcion,inicio_de_actividades,tags,pendiente_revision,tipo,sector,calle,piso_dpto,lat,lon,mail_institucional,telefono,web,responsable_proyecto,mail_responsable,sector_sigla,tipo_sigla,cartodb_georef_status";
        ...
        $SQLQ = "INSERT%20INTO%20mapa_emprendedor%20(". $columnas_db .")%20VALUES%20(" . $geom . ",'" . $campos_post . ",true" .")" . $api_key;
